...
| 구분 | 클래스 역할 | 현재 값 (2025-07 적용분) |
|------|-------------|--------------------------|
| 메인 컨테이너 | 그리드를 감싸는 최상위 div | `uv_uA` |
| 행(Row) | 한 줄(행)을 나타내는 div | `uv_uF` |
| 셀(Cell) | 각 칸(플/뱅/무)을 나타내는 div | `uv_uG` |
...
| 9 | **사용 방법** expander – 복사 명령 예시 (약 721행) | `copy(document.querySelector('.uv_uA').outerHTML);` 에서 `.uv_uA` 부분을 새 메인 컨테이너 클래스명으로 수정 |
...
| 적용 시기 | 메인 컨테이너 | 행 | 셀 |
|-----------|----------------|-----|-----|
| 2025-07-XX (현재) | `uv_uA` | `uv_uF` | `uv_uG` |
| 2025-07-XX | `vh_vm` | `vh_uk` | `vh_vo` |
| 2025-02-XX | `xO_xT` | `xO_xb` | `xO_xV` |
