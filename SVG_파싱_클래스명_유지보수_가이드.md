...
|------|-------------|--------------------------|
| 메인 컨테이너 | 그리드를 감싸는 최상위 div | `rw_rz` |
| 행(Row) | 한 줄(행)을 나타내는 div | `rw_qM` |
| 셀(Cell) | 각 칸(플/뱅/무)을 나타내는 div | `rw_rB` |

**새 HTML 구조 확인 방법:** 브라우저 개발자도구에서 그리드 최상위 div의 `class`를 확인한 뒤, 그 div 기준으로 자식 행/셀의 class를 확인합니다.

---

## 2. 반드시 수정해야 할 위치 (총 7곳)

클래스명이 바뀌면 **아래 7곳을 모두** 새 값으로 맞춰 수정합니다.
...
**검색 키워드:** `[유지보수] 클래스명 변경 시` 또는 `class_='rw_` 로 해당 라인을 찾을 수 있습니다.
...
### 2.3 `change_point/change_point_ngram_app.py` (1곳)

| # | 위치 | 수정 내용 |
|---|------|-----------|
| 7 | **사용 방법** expander 내 문구 (약 712행) | `현재 지원하는 클래스명: \`...\` (메인 컨테이너), \`...\` (행), \`...\` (셀)` 부분을 새 클래스명 3개로 수정 |
...
| 적용 시기 | 메인 컨테이너 | 행 | 셀 |
|-----------|----------------|-----|-----|
| 2025-02-XX (현재) | `rw_rz` | `rw_qM` | `rw_rB` |
...
이후 클래스명이 다시 바뀌면 **이 문서의 체크리스트와 7곳만 참조**하여 동일하게 수정하면 됩니다.
