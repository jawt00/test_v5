...
def validate_first_anchor_window9_freq518_win50_cp(
    ...
    예측 사용 조건: 빈도 기반 신뢰도 ≥ min_confidence_freq(기본 51.8%) 및 시뮬레이션 승률(sim_win_rate_pct) ≥ min_win_rate_pct(기본 50%).
    한쪽이라도 불만족 시 스킵.
    """
    WINDOW_SIZES = (9,)
    conn = get_change_point_db_connection()
...
def batch_validate_first_anchor_window9_10_v2_cp(
    ...
    """
    윈도우 9·10 v2 배치 검증.
    validate_first_anchor_window9_10_v2_cp를 cutoff 이후 모든 grid_string에 대해 호출.
...
def batch_validate_first_anchor_window9_10_v3_cp(
    ...
    validate_first_anchor_window9_10_v3_cp를 cutoff 이후 모든 grid_string에 대해 호출.
...
def batch_validate_first_anchor_window9_10_agree55_cp(
    ...
    validate_first_anchor_window9_10_agree55_cp를 cutoff 이후 모든 grid_string에 대해 호출.
...
def batch_validate_first_anchor_window9_freq518_win50_cp(
    ...
    """
    윈도우 9 (빈도 51.8% + 승률 50%) 배치 검증.
    validate_first_anchor_window9_freq518_win50_cp를 cutoff 이후 모든 grid_string에 대해 호출.
    """
    conn = get_change_point_db_connection()
    try:
        df_test = pd.read_sql_query(
...
def batch_validate_first_anchor_window9_10_agree55_v2_cp(
    ...
    """
    agree55 v2 배치 검증. validate_first_anchor_window9_10_agree55_v2_cp를 cutoff 이후 모든 grid_string에 대해 호출.
    """
    conn = get_change_point_db_connection()
    try:
        df_test = pd.read_sql_query(
