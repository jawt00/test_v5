...
def main():
    ...
    # 마지막 검증데이터: 해당 ID까지만 검증 (선택 안 하면 끝까지)
    end_opts = [None] + df_mw["id"].tolist()
    end_lbl = ["전체 (끝까지)"] + [f"ID {r['id']} · {_fmt_dt(r['created_at'])}" for _, r in df_mw.iterrows()]
    ...
    if test_mode == "단일 테스트":
        ...
        with col2:
            idx_end = st.selectbox(
                "마지막 검증데이터 Grid String (해당 ID까지만 검증)",
                range(len(end_opts)),
                format_func=lambda i: end_lbl[i],
                key="end_select",
                help="선택 시 기준 ID ~ 선택한 ID 구간만 검증. '전체 (끝까지)'면 기준 ID 이후 모두 검증.",
            )
            end_sim = end_opts[idx_end]
        with col3:
        ...
        if cutoff_sim is not None:
            if end_sim is not None:
                st.info(f"📊 **데이터 분리**: ID {cutoff_sim} 이전 = 학습 데이터, **ID {cutoff_sim} ~ ID {end_sim}** = 검증 데이터 (구간만 검증)")
            else:
                st.info(f"📊 **데이터 분리**: ID {cutoff_sim} 이전 = 학습 데이터, ID {cutoff_sim} 이후 = 검증 데이터 (끝까지 검증)")
        else:
            st.info("📊 **데이터 분리**: cutoff를 선택하면 이전은 학습 데이터, 이후는 검증 데이터로 사용됩니다.")
        
        # 예측값 테이블 생성 시 가중치 기반 포함 여부 (기본 비활성화)
        include_weighted = st.checkbox(
            "가중치 기반 예측값도 생성",
            False,
            key="include_weighted_predictions",
            help="체크 시 예측값 테이블 생성 시 빈도 기반과 함께 가중치 기반도 저장합니다. 기본은 빈도 기반만.",
        )
        ...
        if is_threshold_skip_anchor_priority:
            ...
            st.markdown("#### 🔧 시뮬레이션 예측값 테이블 생성 (첫 앵커 확장 윈도우와 동일)")
            st.info(
                "💡 **첫 앵커 확장 윈도우(V3)와 동일한 테이블**을 생성합니다. 윈도우 9~14 저장. 검증 시에는 9·10만 사용합니다. "
                "**시뮬 승률**(sim_win_rate_pct)은 `results_db/sim_results_change_point.db`의 step_events에서 조회하므로, "
                "먼저 시뮬레이션 실행 후 **결과 저장**을 한 뒤 테이블을 생성하면 승률이 채워집니다."
            )
                ...
                if cutoff_sim is None:
                    ...
                    with st.spinner("예측값 테이블 생성 중... (첫 앵커 확장 윈도우와 동일, 9~14)"):
                        try:
                            ...
                            n_wr = result.get("win_rate_entries", 0)
                            st.success(
                                f"✅ 예측값 테이블 생성 완료! (저장된 레코드: {result.get('total_saved', 0):,}개, 시뮬 승률 채움: {n_wr:,}개)"
                            )
                            if n_wr == 0:
                                db_path = result.get("step_events_db_path") or "results_db/sim_results_change_point.db"
                                n_step = result.get("step_events_nonskipped_count")
                                st.info(
                                    "💡 시뮬 승률(sim_win_rate_pct)이 0건입니다. **시뮬레이션 실행 → 결과 저장** 후 다시 예측값 테이블을 생성하세요. "
                                    f"승률은 step_events에서 계산됩니다. 읽은 DB: `{db_path}`"
                                    + (f" (스킵 제외 행: {n_step:,}건)" if n_step is not None else " (행 수 미집계)")
                                )
            ...
            st.info(
                "💡 **첫 앵커 확장 윈도우(V3)와 동일한 테이블**을 생성합니다. 윈도우 9~14 저장. 검증 시에는 9·10만 사용합니다. "
                "**시뮬 승률**은 시뮬레이션 실행 후 **결과 저장**을 한 뒤 테이블을 생성하면 채워집니다."
            )
            ...
            st.info(
                "💡 테이블에 **빈도 기반·가중치 기반** 둘 다 저장되어 있어야 진입 판정이 동작합니다. 윈도우 9~14 저장. "
                "**시뮬 승률**은 시뮬레이션 실행 후 **결과 저장**을 한 뒤 테이블을 생성하면 채워집니다."
            )
        ...
        if st.button("시뮬레이션 실행", type="primary", use_container_width=True):
            if is_first_anchor_extended_v3:
                ...
                if not ws:
                    ...
                    st.session_state["test_end"] = end_sim
        ...
        if selected_hypotheses:
            ...
            with col1:
                ...
                cutoff_sim = cutoff_opts[idx_cutoff]
            with col2:
                idx_end_compare = st.selectbox(
                    "마지막 검증데이터 Grid String (해당 ID까지만 검증)",
                    range(len(end_opts)),
                    format_func=lambda i: end_lbl[i],
                    key="end_select_compare",
                    help="선택 시 기준 ID ~ 선택한 ID 구간만 검증.",
                )
                end_sim_compare = end_opts[idx_end_compare]
            with col3:
                method_sim = st.selectbox("예측 방법", ["빈도 기반", "가중치 기반", "안전 우선"], key="method_compare")
            
            # 데이터 분리 설명
            if cutoff_sim is not None:
                if end_sim_compare is not None:
                    st.info(f"📊 **데이터 분리**: ID {cutoff_sim} 이전 = 학습, **ID {cutoff_sim} ~ ID {end_sim_compare}** = 검증 (구간만)")
                else:
        ...
        if st.button("비교 시뮬레이션 실행", type="primary", use_container_width=True):
            if not selected_hypotheses:
                ...
                st.session_state["test_cutoff"] = cutoff_sim if cutoff_sim is not None else 0
                st.session_state["test_end"] = end_sim_compare
                st.session_state["test_ws"] = ws
                st.session_state["test_method"] = method_sim
                if has_threshold_skip:
    ...
    # 시뮬레이션 실행 (단일/비교)
        ...
        test_end = st.session_state.get("test_end")  # None이면 끝까지, 지정 시 해당 ID까지만 검증
            ...
            try:
                if test_mode == "single":
                    ...
                    if "test_cutoff" in st.session_state:
                        ...
                        if "test_end" in st.session_state:
                            del st.session_state["test_end"]
