...
def main():
    ...
    if not _raw:
        ...
        return
    _priority = ["first_anchor_extended_window_v3", "first_anchor_window9_freq518_win50", "first_anchor_window9_10", "first_anchor_window9_10_agree55_v2", "first_anchor_window9_10_agree55_v3", "first_anchor_window9_only"]
    ...
    if test_mode == "단일 테스트":
        ...
        if is_threshold_skip_anchor_priority:
            ...
            with col_t1:
                weight_unconditional_v2 = st.number_input("가중치 기반 신뢰도 (무조건 사용 임계값, %)", 0.0, 100.0, 72.5, 0.1, key="thresh_agree55_v2_weight_unconditional_clone", help="이 값 이상이면 무조건 가중치 예측값 사용 (0.1 단위)")
            with col_t2:
                thresh_freq_v2 = st.number_input("빈도 신뢰도 최소 (%)", 0.0, 100.0, 52.5, 0.1, key="thresh_agree55_v2_freq_clone")
            with col_t3:
                thresh_weight_v2 = st.number_input("가중치 신뢰도 최소 (%)", 0.0, 100.0, 0.0, 0.1, key="thresh_agree55_v2_weight_clone")
            hypothesis_config = {}
            st.markdown("---")
            ...
            st.info("📌 v2 복제. 규칙 수정 후 시뮬레이션용.")
            ws = [9, 10]
            st.markdown("#### 규칙에 적용되는 신뢰도")
            col_t1, col_t2, col_t3 = st.columns(3)
            with col_t1:
                weight_unconditional_v3 = st.number_input("가중치 기반 신뢰도 (무조건 사용 임계값, %)", 0.0, 100.0, 72.5, 0.1, key="thresh_agree55_v3_weight_unconditional_clone")
            with col_t2:
                thresh_freq_v3 = st.number_input("빈도 신뢰도 최소 (%)", 0.0, 100.0, 52.5, 0.1, key="thresh_agree55_v3_freq_clone")
            with col_t3:
                thresh_weight_v3 = st.number_input("가중치 신뢰도 최소 (%)", 0.0, 100.0, 0.0, 0.1, key="thresh_agree55_v3_weight_clone")
            hypothesis_config = {}
            st.markdown("---")
            st.markdown("#### 🔧 시뮬레이션 예측값 테이블 생성 (필수)")
            if st.button("예측값 테이블 생성", key="generate_agree55_v3_predictions_clone", type="secondary"):
                if cutoff_sim is None:
                    st.warning("기준 Grid String ID를 선택하세요.")
                else:
                    with st.spinner("예측값 테이블 생성 중... (빈도·가중치 9~14, threshold=0)"):
            ...
            st.info("📌 **윈도우 9만** 검증. 빈도 신뢰도 ≥ 51.8% **및** 시뮬레이션 승률 ≥ 50%일 때만 예측 사용.")
            ws = [9]
            st.markdown("#### 규칙에 적용되는 조건")
            col_t1, col_t2 = st.columns(2)
            with col_t1:
                thresh_freq_fw = st.number_input("빈도 신뢰도 최소 (%)", 0.0, 100.0, 51.8, 0.1, key="thresh_freq518_freq_clone")
            with col_t2:
                min_win_rate_fw = st.number_input("시뮬레이션 승률 최소 (%)", 0.0, 100.0, 50.0, 0.1, key="min_win_rate_freq518_clone")
            hypothesis_config = {}
            st.markdown("---")
            st.markdown("#### 🔧 시뮬레이션 예측값 테이블 생성 (필수)")
            if st.button("예측값 테이블 생성", key="generate_freq518_win50_predictions_clone", type="secondary"):
                if cutoff_sim is None:
                    st.warning("기준 Grid String ID를 선택하세요.")
                else:
        ...
        if st.button("시뮬레이션 실행", type="primary", use_container_width=True):
            if is_first_anchor_extended_v3:
                ...
                if cutoff_sim is None:
                    st.warning("기준 Grid String ID를 선택하세요.")
                elif not (st.session_state.get("freq518_win50_predictions_generated", False) or get_simulation_predictions_change_point_count() > 0):
                    st.warning("⚠️ 예측값 테이블이 비어 있거나 시뮬레이션 승률이 없을 수 있습니다. '예측값 테이블 생성'을 먼저 실행하세요.")
                else:
                    st.session_state["test_mode"] = "single"
                    st.session_state["test_hypothesis"] = selected_hypothesis_name
                    st.session_state["test_config"] = hypothesis_config
    ...
    if "test_results" in st.session_state and st.session_state["test_results"] is not None:
        ...
        if st.session_state.get("test_mode") == "single":
            ...
            if not rr:
                ...
                saveable_hypotheses = ("first_anchor_extended_window_v3", "first_anchor_window9_10", "first_anchor_window9_10_agree55_v2", "first_anchor_window9_10_agree55_v3", "first_anchor_window9_freq518_win50")
            ...
            try:
                if test_mode == "single":
                    ...
                    # agree55 v2
                        ...
                        thresh_freq = st.session_state.get("test_thresh_freq", 52.5)
                        thresh_weight = st.session_state.get("test_thresh_weight", 0.0)
                        weight_unconditional = st.session_state.get("test_weight_unconditional_v2", 72.5)
