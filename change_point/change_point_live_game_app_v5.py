...
st.set_page_config(
    page_title="Change-point V5 라이브 게임",
    page_icon="🎮",
    layout="wide",
)
...
def main():
    st.title("🎮 Change-point V5 라이브 게임")
    st.markdown("**V4 복제본 (grid_string 입력 → 게임 시작 → 검증 상세 히스토리)**")

    if "v5_validation_result" not in st.session_state:
        st.session_state.v5_validation_result = None
    ...
    with col_start:
        if st.button("🎮 게임 시작", type="primary", use_container_width=True, key="v5_btn_start"):
            ...
            if not s:
                ...
                with st.spinner("검증 실행 중..."):
                    try:
                        ...
                        st.session_state.v5_validation_result = result
                        st.rerun()
                    except Exception as e:
                        st.error(f"검증 실패: {e}")
    with col_reset:
        if st.button("🔄 초기화", use_container_width=True, key="v5_btn_reset"):
            st.session_state.v5_validation_result = None
            st.rerun()

    result = st.session_state.v5_validation_result
    if result is not None:
        ...
        with col_b:
            if st.button("🔴 B", key="v5_append_b", use_container_width=True):
                ...
                with st.spinner("검증 갱신 중..."):
                    try:
                        ...
                        st.session_state.v5_validation_result = new_result
                        st.rerun()
                    except Exception as e:
                        st.error(f"갱신 실패: {e}")
        with col_p:
            if st.button("🔵 P", key="v5_append_p", use_container_width=True):
