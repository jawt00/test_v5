...
def _query_freq518_win50_prediction(conn, window_size: int, prefix: str, min_freq: float, min_win_rate: float):
    """
    테이블에서 빈도 기반만 조회 후 freq518_win50 조건 적용.
...
# -----------------------------------------------------------------------------
# Step 1: Cold Start — 앵커 추출 → 전수 검증 (freq518_win50 규칙 + 앵커 중첩 시 이전만)
# -----------------------------------------------------------------------------
    ...
    try:
        ...
        while current_pos < len(grid_string) and anchor_idx < len(anchors):
            ...
            for window_size in WINDOW_SIZES:
                ...
                predicted, confidence, skip_reason = _query_freq518_win50_prediction(conn, window_size, prefix, min_confidence_freq, min_win_rate_pct)
...
def predict_next(state: dict, grid_string: str):
    """다음 포지션(len(grid_string))에 대한 freq518_win50 예측 조회."""
    ...
    conn = get_change_point_db_connection()
    try:
        predicted, confidence, skip_reason = _query_freq518_win50_prediction(conn, window, prefix, min_freq, min_wr)
...
def main():
    ...
    with col_freq:
        min_freq = st.number_input("빈도 신뢰도 최소 (%)", 0.0, 100.0, float(st.session_state.flow_freq518_win50_saved_freq), 0.1, key="freq518_min_freq", help="빈도 기반 confidence가 이 값 이상일 때만 예측 사용")
    with col_wr:
        min_win_rate = st.number_input("시뮬레이션 승률 최소 (%)", 0.0, 100.0, float(st.session_state.flow_freq518_win50_saved_win_rate), 0.1, key="freq518_min_win_rate", help="sim_win_rate_pct가 이 값 이상일 때만 예측 사용")
    ...
    with col_save:
        if st.button("💾 설정 저장", type="secondary", use_container_width=True, key="freq518_btn_save"):
            ...
            st.rerun()
    with col_show:
        st.caption(f"**저장된 조건:** 빈도 **{st.session_state.flow_freq518_win50_saved_freq}%** · 승률 **{st.session_state.flow_freq518_win50_saved_win_rate}%**")
        ...
        if st.button("🎮 게임 시작 (Cold Start)", type="primary", use_container_width=True, key="freq518_btn_start"):
            ...
            if not s:
                ...
                with st.spinner("Cold Start 검증 실행 중..."):
                    try:
                        result = cold_start(s, min_confidence_freq=st.session_state.flow_freq518_win50_saved_freq, min_win_rate_pct=st.session_state.flow_freq518_win50_saved_win_rate)
