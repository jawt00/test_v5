...
BEAD_ROAD_MAIN_CONTAINER_CLASS = getattr(_svg_parser, 'BEAD_ROAD_MAIN_CONTAINER_CLASS', 'rN_rQ')
...
def main():
    ...
    with st.expander("ℹ️ 사용 방법", expanded=False):
        ...
        1. **SVG 코드 입력 및 파싱**
           ...
           - 현재 지원하는 클래스명: `rN_rQ` (메인 컨테이너), `rN_oT` (행), `rN_rS` (셀)
           - 브라우저에서 그리드 복사: 개발자도구 콘솔에 `copy(document.querySelector('.rN_rQ').outerHTML);` 실행
