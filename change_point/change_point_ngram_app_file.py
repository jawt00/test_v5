...
def main():
    ...
    with st.expander("ℹ️ 사용 방법", expanded=False):
        ...
        1. **HTML 파일 업로드 및 파싱**
           ...
           - 현재 지원하는 클래스명: `{getattr(_svg_parser, 'BEAD_ROAD_MAIN_CONTAINER_CLASS', 'uS_uX')}` (메인), `{getattr(_svg_parser, 'BEAD_ROAD_ROW_CLASS', 'uS_uq')}` (행), `{getattr(_svg_parser, 'BEAD_ROAD_CELL_CLASS', 'uS_uZ')}` (셀)
