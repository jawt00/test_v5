"""
Change-point Detection 기반 N-gram 생성 앱 (HTML 파일 업로드)
HTML 파일 전체에서 Bead Road 그리드를 찾아 파싱
"""
...
# 페이지 설정
st.set_page_config(
    page_title="Change-point N-gram Generator (HTML File)",
    page_icon="🔍",
    layout="wide"
)
...
def main():
    st.title("🔍 Change-point Detection 기반 N-gram 생성 (HTML 파일)")
    st.markdown("HTML 파일 전체에서 Bead Road 그리드를 찾아 파싱하고, 변화점 기반 N-gram을 생성합니다.")
    st.markdown("---")
    ...
    uploaded_file = st.file_uploader(
        "HTML 파일 선택",
        type=["html", "htm"],
        help="브라우저에서 저장한 .html 파일을 업로드하세요.",
        key=f"html_uploader_{st.session_state.html_input_key_counter}",
    )
    
    if uploaded_file is not None:
        try:
            st.session_state.html_source = _read_uploaded_html(uploaded_file)
            st.session_state.html_filename = uploaded_file.name
    ...
    with col_svg2:
        ...
        parse_button = st.button("🔍 파싱", type="primary", use_container_width=True, key="parse_html_button")
    ...
    with col_svg2:
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("🔄 리셋", use_container_width=True, key="reset_html_input_button"):
            st.session_state.html_input_key_counter += 1
            for key in (
                'html_source', 'html_filename', 'parsed_grid_string', 'parsed_grid',
    ...
    with col_svg1:
        if st.session_state.get('html_filename'):
            st.info(f"선택된 파일: **{st.session_state.html_filename}** — '파싱' 버튼을 클릭하세요.")
        else:
            st.info("HTML 파일을 선택한 후 '파싱' 버튼을 클릭하세요.")
    
    # 파싱 실행
    html_content = st.session_state.get('html_source', '')
    if parse_button:
        if not html_content or not html_content.strip():
            st.warning("⚠️ HTML 파일을 선택해주세요.")
        else:
            ...
            with st.spinner("HTML 파싱 중..."):
                try:
                    parsed_grid = parse_bead_road_svg(html_content)
    ...
    if 'parsing_error' in st.session_state:
        st.error(f"❌ HTML 파싱 중 오류 발생: {st.session_state.parsing_error}")
    ...
    with st.expander("ℹ️ 사용 방법", expanded=False):
        st.markdown(f"""
        ### Change-point Detection 기반 N-gram 생성 앱 (HTML 파일) 사용 방법
        
        1. **HTML 파일 업로드 및 파싱**
           - 브라우저에서 "페이지 저장" 또는 개발자도구로 저장한 `.html` 파일 업로드
           - '파싱' 클릭 → 파일 전체에서 Bead Road 컨테이너 (`.{BEAD_ROAD_MAIN_CONTAINER_CLASS}`) 자동 탐색
           - 현재 지원하는 클래스명: `{getattr(_svg_parser, 'BEAD_ROAD_MAIN_CONTAINER_CLASS', 'xO_xT')}` (메인), `{getattr(_svg_parser, 'BEAD_ROAD_ROW_CLASS', 'xO_xb')}` (행), `{getattr(_svg_parser, 'BEAD_ROAD_CELL_CLASS', 'xO_xV')}` (셀)
