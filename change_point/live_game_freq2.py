"""
윈도우 9 전용 (빈도 + 시뮬 승률) 라이브 게임 앱 — 조건 1·2 병렬 승인(OR).

- Cold Start → State Handoff → Live Loop (기존 플로우와 동일).
- 조건 1 또는 조건 2 중 하나만 만족해도 예측 사용 (OR). 하나만 통과하면 베팅 가능.
- 예측: simulation_predictions_change_point에서 빈도 기반만 조회 (테이블 threshold=0).
...
# 조건 2 기본값 (조건 1·2 모두 적용 시 사용)
DEFAULT_COND2_FREQ = 53.2
DEFAULT_COND2_WIN_RATE = 46.5
...
def _query_freq518_win50_prediction(conn, window_size: int, prefix: str, c1_freq: float, c1_wr: float, c2_freq: float, c2_wr: float):
    """
    테이블에서 빈도 기반만 조회 후 OR 조건 적용.
    조건 1 (c1_freq, c1_wr) 또는 조건 2 (c2_freq, c2_wr) 중 하나만 만족해도 예측 사용.
    ...
    # OR 조건: 조건 1 만족 또는 조건 2 만족 시 예측 사용
    cond1_ok = conf_freq >= c1_freq and sim_wr >= c1_wr
    cond2_ok = conf_freq >= c2_freq and sim_wr >= c2_wr
    if cond1_ok or cond2_ok:
        return pred_val, conf_freq, None
    return pred_val, conf_freq, f"조건 1·2 모두 미충족 (빈도 {conf_freq:.1f}%, 승률 {sim_wr:.1f}%)"


# -----------------------------------------------------------------------------
# Step 1: Cold Start — 앵커 추출 → 전수 검증 (OR 조건: 조건 1 또는 조건 2 만족 시 예측)
# -----------------------------------------------------------------------------
def cold_start(
    grid_string: str,
    min_confidence_freq: float = MIN_CONFIDENCE_FREQ,
    min_win_rate_pct: float = MIN_WIN_RATE_PCT,
    min_confidence_freq2: float = DEFAULT_COND2_FREQ,
    min_win_rate_pct2: float = DEFAULT_COND2_WIN_RATE,
):
    """
    윈도우 9만. 조건 1 또는 조건 2 중 하나만 만족해도 예측 사용 (OR).
    앵커 중첩 시 validated_positions로 이전 앵커만 검증.
    """
    def _base_state():
        return {
            "current_pos": 0, "active_anchor_idx": 0, "anchor_failure_count": 0, "next_window_size": 9,
            "anchors": [], "search_from": 0,
            "min_confidence_freq": min_confidence_freq, "min_win_rate_pct": min_win_rate_pct,
            "min_confidence_freq2": min_confidence_freq2, "min_win_rate_pct2": min_win_rate_pct2,
        }
    min_ws = min(WINDOW_SIZES)
    if len(grid_string) < MIN_GRID_LENGTH:
        return {"state": _base_state(), "history": [], "grid_string": grid_string, "summary": {"total_steps": 0, "total_failures": 0, "total_predictions": 0, "total_skipped": 0, "accuracy": 0.0}}
    ...
    try:
        ...
        while current_pos < len(grid_string) and anchor_idx < len(anchors):
            ...
            for window_size in WINDOW_SIZES:
                ...
                predicted, confidence, skip_reason = _query_freq518_win50_prediction(conn, window_size, prefix, min_confidence_freq, min_win_rate_pct, min_confidence_freq2, min_win_rate_pct2)
        ...
        state = {
            ...
            "min_confidence_freq2": min_confidence_freq2,
            "min_win_rate_pct2": min_win_rate_pct2,
        }
...
# -----------------------------------------------------------------------------
# Live Loop — 예측 노출 / 단일 스텝 검증 (freq518_win50)
# -----------------------------------------------------------------------------
def predict_next(state: dict, grid_string: str):
    """다음 포지션(len(grid_string))에 대한 예측 조회. 조건 1 또는 조건 2 만족 시 예측 사용 (OR)."""
    ...
    c1_freq = state.get("min_confidence_freq", MIN_CONFIDENCE_FREQ)
    c1_wr = state.get("min_win_rate_pct", MIN_WIN_RATE_PCT)
    c2_freq = state.get("min_confidence_freq2", DEFAULT_COND2_FREQ)
    c2_wr = state.get("min_win_rate_pct2", DEFAULT_COND2_WIN_RATE)
    conn = get_change_point_db_connection()
    try:
        predicted, confidence, skip_reason = _query_freq518_win50_prediction(conn, window, prefix, c1_freq, c1_wr, c2_freq, c2_wr)
...
def live_step(state: dict, grid_string: str, history: list, user_input: str):
    ...
    if skipped or predicted is None:
        ...
        new_state = {
            ...
            "min_confidence_freq2": state.get("min_confidence_freq2", DEFAULT_COND2_FREQ),
            "min_win_rate_pct2": state.get("min_win_rate_pct2", DEFAULT_COND2_WIN_RATE),
        }
...
def main():
    ...
    if "flow_freq518_win50_saved_freq" not in st.session_state:
        st.session_state.flow_freq518_win50_saved_freq = MIN_CONFIDENCE_FREQ
    if "flow_freq518_win50_saved_win_rate" not in st.session_state:
        st.session_state.flow_freq518_win50_saved_win_rate = MIN_WIN_RATE_PCT
    if "flow_freq518_win50_saved_freq2" not in st.session_state:
        st.session_state.flow_freq518_win50_saved_freq2 = DEFAULT_COND2_FREQ
    if "flow_freq518_win50_saved_win_rate2" not in st.session_state:
        st.session_state.flow_freq518_win50_saved_win_rate2 = DEFAULT_COND2_WIN_RATE

    # 설정: 조건 1 / 조건 2 (OR — 하나만 통과해도 예측·베팅)
    st.markdown("### 1. 조건 설정 (OR: 조건 1 또는 조건 2 하나만 만족해도 예측·베팅)")
    st.markdown("**조건 1**")
    col_freq, col_wr = st.columns(2)
    with col_freq:
        min_freq = st.number_input("조건 1 · 빈도 신뢰도 최소 (%)", 0.0, 100.0, float(st.session_state.flow_freq518_win50_saved_freq), 0.1, key="freq518_min_freq", help="빈도 기반 confidence가 이 값 이상일 때만 예측 사용")
    with col_wr:
        min_win_rate = st.number_input("조건 1 · 시뮬레이션 승률 최소 (%)", 0.0, 100.0, float(st.session_state.flow_freq518_win50_saved_win_rate), 0.1, key="freq518_min_win_rate", help="sim_win_rate_pct가 이 값 이상일 때만 예측 사용")

    st.markdown("**조건 2**")
    col_freq2, col_wr2 = st.columns(2)
    with col_freq2:
        min_freq2 = st.number_input("조건 2 · 빈도 신뢰도 최소 (%)", 0.0, 100.0, float(st.session_state.flow_freq518_win50_saved_freq2), 0.1, key="freq518_min_freq2", help="조건 1과 함께 적용 (둘 다 만족해야 함)")
    with col_wr2:
        min_win_rate2 = st.number_input("조건 2 · 시뮬레이션 승률 최소 (%)", 0.0, 100.0, float(st.session_state.flow_freq518_win50_saved_win_rate2), 0.1, key="freq518_min_win_rate2", help="조건 1과 함께 적용 (둘 다 만족해야 함)")

    st.caption("**적용 기준 (OR):** 조건 1 또는 조건 2 중 **하나만 만족**해도 예측 사용 · 베팅 가능")

    col_save, col_show, _ = st.columns([1, 2, 3])
    with col_save:
        if st.button("💾 설정 저장", type="secondary", use_container_width=True, key="freq518_btn_save"):
            st.session_state.flow_freq518_win50_saved_freq = min_freq
            st.session_state.flow_freq518_win50_saved_win_rate = min_win_rate
            st.session_state.flow_freq518_win50_saved_freq2 = min_freq2
            st.session_state.flow_freq518_win50_saved_win_rate2 = min_win_rate2
            st.success("저장되었습니다.")
            st.rerun()
    with col_show:
        st.caption(
            f"**저장된 조건 1:** 빈도 {st.session_state.flow_freq518_win50_saved_freq}% · 승률 {st.session_state.flow_freq518_win50_saved_win_rate}% | "
            f"**조건 2:** 빈도 {st.session_state.flow_freq518_win50_saved_freq2}% · 승률 {st.session_state.flow_freq518_win50_saved_win_rate2}%"
        )
    st.caption("저장 후 게임 시작 시 조건 1 또는 조건 2 중 하나만 만족해도 예측 사용 (OR)으로 Cold Start가 실행됩니다.")
    ...
    with col_start:
        if st.button("🎮 게임 시작 (Cold Start)", type="primary", use_container_width=True, key="freq518_btn_start"):
            ...
            if not s:
                ...
                with st.spinner("Cold Start 검증 실행 중..."):
                    try:
                        result = cold_start(
                            s,
                            min_confidence_freq=st.session_state.flow_freq518_win50_saved_freq,
                            min_win_rate_pct=st.session_state.flow_freq518_win50_saved_win_rate,
                            min_confidence_freq2=st.session_state.flow_freq518_win50_saved_freq2,
                            min_win_rate_pct2=st.session_state.flow_freq518_win50_saved_win_rate2,
                        )
                        st.session_state.flow_freq518_win50_result = result
                        st.rerun()
                    except Exception as e:
                        st.error(f"Cold Start 실패: {e}")
    with col_reset:
        if st.button("🔄 초기화", use_container_width=True, key="freq518_btn_reset"):
            st.session_state.flow_freq518_win50_result = None
            st.session_state.pop("flow_freq518_win50_last_saved", None)
            st.rerun()

    result = st.session_state.flow_freq518_win50_result
    if result is not None:
        ...
        cfg_freq2 = state.get("min_confidence_freq2", DEFAULT_COND2_FREQ)
        cfg_wr2 = state.get("min_win_rate_pct2", DEFAULT_COND2_WIN_RATE)
        st.caption(f"윈도우 9만 사용 · OR: (빈도≥{cfg_freq}%·승률≥{cfg_wr}%) **또는** (빈도≥{cfg_freq2}%·승률≥{cfg_wr2}%)")
        ...
        with col_b:
            if st.button("🔴 B", key="freq518_append_b", use_container_width=True):
                try:
                    step_out = live_step(state, gs, history, "b")
                    st.session_state.flow_freq518_win50_result = {
        ...
        with col_p:
            if st.button("🔵 P", key="freq518_append_p", use_container_width=True):
                try:
        ...
        st.markdown("#### 결과 저장")
        if st.session_state.get("flow_freq518_win50_last_saved"):
            last = st.session_state.flow_freq518_win50_last_saved
            st.success(f"**저장됨** · run_id: `{last.get('run_id', '')}`")
        if st.button("결과 저장", key="freq518_save_results", type="secondary", use_container_width=True):
            try:
                ...
                st.session_state.flow_freq518_win50_last_saved = {"run_id": run_id}
                st.rerun()
            except Exception as e:
                st.error(f"저장 실패: {e}")
