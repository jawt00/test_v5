"""
[복제본2 - 이 파일만 수정] Cold Start → State Handoff → Live Loop 플로우 전용 라이브 게임 앱 (3가지 검증 방식).
...
st.set_page_config(
    page_title="라이브 게임 쓰리모드 V2 (가중치 기반)",
    page_icon="🎮",
    layout="wide",
)
...
def build_validation_history_table_by_position(flow_result: dict) -> list:
    """포지션 0부터 max까지 한 행에 한 포지션, 열: Position, W9_10_예측/실제/일치, V3_예측/실제/일치, W9_예측/실제/일치."""
    results = flow_result.get("results") or {}
    gs = flow_result.get("grid_string") or ""
    by_pos = {}
    for mode in MODES:
...
def main():
    ...
    st.info("📌 **예측 모델: 가중치 기반** (V2 — 이 앱만 가중치 기반 예측 사용)")
