"""
[V3 - 이 파일만 수정] Cold Start → State Handoff → Live Loop 플로우 전용 라이브 게임 앱 (3가지 검증 방식).
...
st.set_page_config(
    page_title="Change-point 플로우 라이브 게임 v3 (3가지 검증 방식)",
    page_icon="🎮",
    layout="wide",
)
...
def build_current_state_table(flow_result: dict) -> list:
    ...
    for mode in DISPLAY_ORDER:
        ...
        row[_mode_label(mode)] = pv if (not pred.get("skipped") and pv) else "-"
    return [row]
...
def build_validation_history_table_by_position(flow_result: dict) -> list:
    ...
    for pos in range(0, max_pos + 1):
        d = by_pos.get(pos, {})
        w910_ok = d.get("window9_10", {}).get("is_correct")
        v3_ok = d.get("v3", {}).get("is_correct")
        w9_ok = d.get("window9", {}).get("is_correct")
        # 해당 포지션 행이 모두 스킵(일치가 O/X 아님)이면 테이블에서 숨김
        match_w910 = "O" if w910_ok else ("X" if w910_ok is False else "-")
        match_v3 = "O" if v3_ok else ("X" if v3_ok is False else "-")
        match_w9 = "O" if w9_ok else ("X" if w9_ok is False else "-")
        if match_w910 == "-" and match_v3 == "-" and match_w9 == "-":
            continue
        row = {
            "Position": pos,
...
def main():
    st.title("Change-point 플로우 라이브 게임 v3 (3가지 검증 방식)")
        ...
        if st.button("결과 저장", key="flow_save_results_three", type="secondary", use_container_width=True):
            try:
                run_id = save_live_run_results(
                    ...
                    history_by_mode={m: results.get(m, {}).get("history") or [] for m in MODES},
                    summary_by_mode={m: results.get(m, {}).get("summary") or _empty_summary() for m in MODES},
                )
