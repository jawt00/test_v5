"""
[V4 - 이 파일만 수정] Cold Start → State Handoff → Live Loop 플로우 전용 라이브 게임 앱 (3가지 검증 방식).

- V3 (9~14), 윈도우 9 전용, 윈도우 9·10 세 모드를 병렬 실행.
- 현재 상태: 현재 포지션에 대해 3가지 검증별 예측값 있음/없음 테이블.
- 검증 히스토리: 포지션 0부터 전체, 1개 테이블에 V3/W9/W9_10 열로 표시.
- v3 복제본 → v4에서 수정용.
- 예측값: change_point/pattern_list2.db · simulation_predictions_change_point
- 결과 저장: change_point/pattern_list2.db · live_step_results (스텝 상세 누적)
"""
...
PREDICTIONS_DB_PATH = Path(__file__).resolve().parent / "pattern_list2.db"
...
def save_live_step_results(history: list, saved_keys: set):
    """
    아직 저장되지 않은 스텝만 pattern_list2.db.live_step_results에 누적 INSERT.
...
def main():
    st.title("Change-point 플로우 라이브 게임 v4 (3가지 검증 방식)")
    st.markdown("**Cold Start → State Handoff → Live Loop** · W9_10 / V3 / 윈도우 9 병렬 실행")
    st.caption(
        f"예측 DB: `{PREDICTIONS_DB_PATH.name}` · `{PREDICTIONS_TABLE}` · "
        f"결과 저장: `{LIVE_STEP_RESULTS_TABLE}` (스텝 누적)"
    )
