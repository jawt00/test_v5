...
def build_prediction_history_wide(
    ...
    *,
    ...
    Returns:
        (pred_wide, rule_wide, column_meta)
    ...
    try:
        ...
        if runs_df.empty:
            snaps = pd.DataFrame(
                columns=["run_id", "prefix", "predicted_value", "final_rule"]
            )
        else:
    ...
    if runs_df.empty:
        ...
        return pred_wide.reset_index(drop=True), rule_wide.reset_index(drop=True), meta
