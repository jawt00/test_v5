...
def ensure_snapshot_schema(conn: sqlite3.Connection) -> None:
    ...
    migrate_sim_extra_columns(conn, TABLE_SNAPSHOTS)
    conn.commit()
        ...
        if not snapshot_df.empty:
            insert_sql = f"""
                INSERT INTO {TABLE_SNAPSHOTS} (
                    ...
                    rule_version, agree_count, agree_count_new_three,
            ...
            records = [
                (
                    ...
                    if pd.notna(getattr(r, "rule_confidence", None))
                    else None,
                    getattr(r, "conf_source", None),
                    float(r.min_agree_conf)
                    if pd.notna(getattr(r, "min_agree_conf", None))
                    else None,
                    float(r.mean_agree_conf)
                    if pd.notna(getattr(r, "mean_agree_conf", None))
                    else None,
...
def capture_snapshot_after_refresh(
    ...
    *,
    ...
    if cmp_df.empty:
        return None
    rv = rule_version or RULE_VERSION
    pred_df = build_simulation_predictions_df(
        cmp_df,
        profile,
        final_pred_fn,
...
def restore_snapshot_to_current(profile: PatternListProfile, run_id: str) -> int:
    ...
    pred_df = snap_df.copy()
    for col in _INSERT_COLS:
        if col not in pred_df.columns:
            pred_df[col] = None
    pred_df = pred_df[_INSERT_COLS]
