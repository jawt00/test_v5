...
def generate_missing_ngrams(target_window_sizes=[10, 11, 12]):
    ...
    print()
    print(f"누락된 윈도우 크기 {target_window_sizes}로 N-gram 생성합니다...")
    print()
            ...
            try:
                ...
                if not missing_window_sizes:
                    ...
                    if skipped_count <= 5:  # 처음 5개만 출력
                        print(f"ID {record_id}: 모든 윈도우 크기({target_window_sizes})가 이미 존재합니다. (건너뜀)")
                    elif skipped_count == 6:
                        print("  ... (더 많은 건너뛴 항목이 있습니다)")
                    continue
...
if __name__ == "__main__":
    print("=" * 60)
    print("누락된 윈도우 크기 N-gram 생성 스크립트")
    print("=" * 60)
    print()
